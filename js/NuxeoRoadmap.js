...
function formatDate(dateToFormat){
	...
	function formatVersions(jSONVersions){
		...
		for(i=0;i<jSONVersions.length;i++){
			...
			var classAttribute="";
		    if (nextCurrentVersion) {panelClass="alert alert-info"; nextCurrentVersion=false};
            if (jSONVersions[i].released) {currentVersion=false;nextCurrentVersion=true;panelClass="alert alert-success"};
		...
		for(i=0;i<jSONVersions.length;i++){
			if(i==0){htmlOutput=htmlOutput+" <div class=\"tab-pane fade in active\" id=\"version-"+jSONVersions[i].id+"\"></div>"}
				else{htmlOutput=htmlOutput+" <div class=\"tab-pane fade \" id=\"version-"+jSONVersions[i].id+"\"></div>"}
		}
...
result ="<div class=\"panel-group\" id=\"accordion\">";
 for (i=0;i<roadmapItemsList.length;i++){
		...
		if (roadmapItem.fields.resolution!==null&&roadmapItem.fields.resolution.name=="Fixed"){isRoadmapItemResolved=true;};
